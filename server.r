...
shinyServer(function(input, output) {
  ...
  test_19_covid_Confirmed$date <-
    lubridate::parse_date_time(sub("X", "", test_19_covid_Confirmed$Calendar_Date),
                               orders = "mdy")
  
  test_19_covid_Recovered$date <-
    lubridate::parse_date_time(sub("X", "", test_19_covid_Recovered$Calendar_Date),
                               orders = "mdy")
  ...
  test_19_covid_Confirmed_bar$date <-
    lubridate::parse_date_time(sub("X", "", test_19_covid_Confirmed_bar$Calendar_Date),
                               orders = "mdy")
  
  test_19_covid_Recovered_bar$date <-
    lubridate::parse_date_time(sub("X", "", test_19_covid_Recovered_bar$Calendar_Date),
                               orders = "mdy")
  
  test_19_covid_Deaths_bar$date <-
    lubridate::parse_date_time(sub("X", "", test_19_covid_Deaths_bar$Calendar_Date),
                               orders = "mdy")
  ...
  #  valueBox(
  #    formatC(all_19_covid_today$Headcount_Confirmed, format="d", big.mark=',')
  #    ,paste('Total Confirmed')
  ...
  output$test <- renderPlot({
    all_19_covid_today <- all_19_covid %>% filter(date.x >= input$date[1] &
                                                    date.x <= input$date[1])
    ...
    gg <-
      gg + geom_point(
        data = subset(all_19_covid_today, Headcount_Confirmed > 0),
    ...
    gg <-
      gg + geom_point(
        data = subset(all_19_covid_today, Headcount_Confirmed > 5000),
        aes(x = Long, y = Lat, size = Headcount_Confirmed),
        shape = 1,
        alpha = 1,
        color = "black",
    ...
    gg <-
      gg +  geom_scatterpie(
        data = subset(all_19_covid_today, Headcount_Confirmed > 500),
        aes(
          x = Long,
          y = Lat,
          r = sqrt(Headcount_Confirmed) / 50
        ),
        cols = c(
          "Headcount_active",
          "Headcount_Recovered",
    ...
    output$WHOTimeline <- renderPlot({
      WHO_timeline <-
        read.csv(
          "https://raw.githubusercontent.com/Z-ingdotnet/COVID19_Dashboard/master/WHO%20timeline.csv"
        )
      
      status_levels <-
        c("Minor", "Significant", "Severe", "Emergency")
      ...
      df$status <-
        factor(df$status, levels = status_levels, ordered = TRUE)
      ...
      df$text_position <-
        (df$month_count * text_offset * df$direction) + df$position
      ...
      timeline_plot <-
        timeline_plot + scale_color_manual(values = status_colors,
      ...
      timeline_plot + labs(
        ...
        caption = i18n()$t(
          "Source: https://www.who.int/news-room/detail/08-04-2020-who-timeline---covid-19"
        )
      )
    })
    ...
    output$COVID_19_timeseries <- renderPlot({
      ...
      gg <-
        gg + geom_line(aes(y = Headcount_Recovered, col = i18n()$t("Recovered")), size = 2)
      gg <-
        gg + geom_line(aes(y = Headcount_active, col = i18n()$t("Active")), size = 2)
      gg <-
        gg + geom_line(aes(y = Headcount_Confirmed, col = i18n()$t("Confirmed")), size = 1)
      gg <-
        gg + geom_line(aes(y = Headcount_Deaths, col = i18n()$t("Deaths")), size = 1)
      gg <-
        gg + scale_x_date(breaks = '1 week', date_labels = "%d-%b-%y")
      gg <- gg + xlab(i18n()$t("Date"))
      gg <-
        gg + ylab(i18n()$t("Number of People")) + scale_y_continuous(labels = scales::comma)
      gg <- gg + scale_color_manual(name = "",
                                    values = c("#e82507", "#E7B800", "#0a0a0a", "#22d606"))
      gg <-
        gg + theme(panel.grid.minor = element_blank(),
                   axis.text.x = element_text(angle = 90, hjust = 1))
      gg
    ...
    output$Cumulative_COVID_19 <- renderPlot({
      all_19_covid_bar <-
        rbind(
          test_19_covid_Active_bar,
          test_19_covid_Recovered_bar,
          test_19_covid_Deaths_bar
        )
      
      
      qualitative <- c("#e41a1c"
                       , "#000000"
                       , "#4daf4a")
      
      gg2 <-
        ggplot(data = all_19_covid_bar, aes(
          x = as.Date(date),
          y = Headcount,
          fill = Type
        ))
      gg2 <-
        gg2 + geom_bar(stat = "identity", position = 'stack'#， position = 'dodge') +
                       ...
                       gg2 <-
                         gg2 + xlab(i18n()$t("Date")) + ylab(i18n()$t("Number of People")) + scale_y_continuous(labels = scales::comma)
                       gg2 <-
                         gg2 + theme(panel.grid.minor = element_blank(),
                                     axis.text.x = element_text(angle = 90, hjust = 1))
                       gg2
      ...
      output$COVID_19_timeseries_World_China <- renderPlot({
        ...
        all_19_covid_line2 <-
          merge(all_19_covid_line_china,
                all_19_covid_line_nonchina,
                by = c("date"))
        names(all_19_covid_line2)[4] <- "Confirmed_china"
        names(all_19_covid_line2)[8] <- "Confirmed_outsidechina"
        all_19_covid_line2 <-
          merge(all_19_covid_line[, c(1:3)], all_19_covid_line2, by = c("date"))
        ...
        gg <-
          gg +  geom_line(aes(
            y = Confirmed_china,
            col = i18n()$t("Total Confirmed Mainland China")
          ), size = 1.5)
        gg <-
          gg +  geom_line(aes(
            y = Confirmed_outsidechina,
            col = i18n()$t("Total Confirmed Outside Mainland China")
          ), size = 1.5)
        gg <-
          gg +  geom_line(aes(y = Headcount_active, col = i18n()$t("Active Worldwide")), size = 1.5)
        gg <-
          gg +  geom_line(aes(
            y = Headcount_Recovered,
            col = i18n()$t("Total Recovered Worldwide")
          ), size = 1.5)
        #ggtitle("COVID-19 timeseries Worldwide & Mainland China")
        gg <-
          gg +  xlab(i18n()$t("Date")) + ylab(i18n()$t("Number of People"))
        gg <-
          gg + scale_x_date(breaks = '1 week', date_labels = "%d-%b-%y")
      ...
      output$COVID_19_timeseries_World_China_log <- renderPlot({
        ...
        gg <-
          gg +   geom_line(aes(
            y = log(Confirmed_china),
            col = i18n()$t("Total Confirmed Mainland China")
          ), size = 1.5)
        gg <-
          gg + geom_line(aes(
            y = log(Confirmed_outsidechina),
            col = i18n()$t("Total Confirmed Outside Mainland China")
          ), size = 1.5)
        gg <-
          gg +  geom_line(aes(
            y = log(Headcount_active),
            col = i18n()$t("Active Worldwide")
          ), size = 1.5)
        gg <-
          gg +  geom_line(aes(
            y = log(Headcount_Recovered),
            col = i18n()$t("Total Recovered Worldwide")
          ), size = 1.5)
